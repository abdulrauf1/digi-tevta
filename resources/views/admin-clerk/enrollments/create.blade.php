<x-app-layout>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('enrollments.store') }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <!-- Trainee Selection -->
                            <div>
                                <label for="trainee_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    Trainee *
                                </label>
                                <select name="trainee_id" id="trainee_id" required
                                    ...
                                    @foreach($trainees as $trainee)
                                        <option value="{{ $trainee->id }}" {{ old('trainee_id') == $trainee->id ? 'selected' : '' }}>
                                            {{ $trainee->user->name }} - {{ $trainee->cnic }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('trainee_id')
                        ...
                        <div class="mb-6">
                            ...
                            <select name="status" id="status" required
                                ...
                                <option value="altered" {{ old('status') == 'altered' ? 'selected' : '' }}>Altered</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Check for existing enrollment when session or trainee changes
            ...
            sessionSelect.addEventListener('change', checkExistingEnrollment);
            traineeSelect.addEventListener('change', checkExistingEnrollment);
        });
    </script>
    @endpush
</x-app-layout>